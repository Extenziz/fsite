...
function typeWriter(element, text, speed, callback) {
    ...
    function type() {
        if (index < text.length) {
            ...
            callback(); // Wywołaj kolejną funkcję po zakończeniu wpisywania
        }
    }
    type();
}

// Zaczynamy efekt wpisywania po załadowaniu strony
document.addEventListener("DOMContentLoaded", function() {
    const headerElement = document.getElementById('typed-header');
    const textElement = document.getElementById('typed-text');

    // Wpisz nagłówek
    typeWriter(headerElement, headerText, 50, function() {  // Szybsze wpisywanie nagłówka (50ms)
        // Po zakończeniu wpisywania nagłówka wpisz tekst manifestu
        typeWriter(textElement, manifestText, 1);  // Szybsze wpisywanie tekstu (30ms)
    });
});